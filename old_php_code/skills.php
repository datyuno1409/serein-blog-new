    ...
    switch ($method) {
        case 'GET':
            if (isset($_GET['id'])) {
                ...
                $result = $skill->getByAboutId($_GET['about_id']);
                echo json_encode(['success' => true, 'data' => $result]);
            } else {
                $result = $skill->all();
                echo json_encode(['success' => true, 'data' => $result]);
        ...
        case 'POST':
            ...
            $result = $skill->create($input);
            if ($result) {
                http_response_code(201);
                echo json_encode(['success' => true, 'data' => $result, 'message' => 'Skill created successfully']);
            } else {
        ...
        case 'PUT':
            ...
            if (isset($input['order'])) {
                $result = $skill->updateOrder($_GET['id'], $input['order']);
                if ($result) {
                    echo json_encode(['success' => true, 'message' => 'Skill order updated successfully']);
                } else {
                    http_response_code(500);
                    echo json_encode(['success' => false, 'message' => 'Failed to update skill order']);
                }
                break;
            }
            ...
            $result = $skill->update($_GET['id'], $input);
            if ($result) {
